...
def get_border_and_priority_analytics():
    redis_class = Redis()
    route_data = redis_class.get_data(route_num='2')
    if route_data:
        return route_data
    else:
        mongo_class = MongoConnection()
        collection = mongo_class.get_collection()
        ...
        redis_class.insert_route_data(route, '2')
        if route_data:
            return route
        else:
            return None
...
def get_5_top_urgent_zones():
    redis_class = Redis()
    route_data = redis_class.get_data(route_num='1')
    if route_data:
        return route_data
    else:
        mongo_class = MongoConnection()
        ...
        redis_class.insert_route_data(route, '1')
        if route_data:
            return route
        else:
            return None
...
def get_distance_distribution_analytics():
    redis_class = Redis()
    route_data = redis_class.get_data(route_num='3')
    if route_data:
        return route_data
    else:
        mongo_class = MongoConnection()
        ...
        redis_class.insert_route_data(route, '3')
        if route_data:
            return route
        else:
            return None
